...
BitMap::BitMap(string left_path, string right_path)
{
    ...
    this->bmpHandle.applyCDFOnImages(true);
...
void BitMap::sobelTemplateMatch(int rowNo, int patchSize, int rowOffset, int edgeLineNo)
{
    ...
    for (int edgeNo = 0; edgeNo < this->leftEdgePoints[edgeLineNo].size(); edgeNo++)
    {
        ...
        if (((255 * 255 * patchSize * patchSize * 0.01) >= best_ssd) &&
            (this->bmpHandle.abs(best_coord.second - leftEdgePoints[edgeLineNo][edgeNo].second) < this->getWidth() / 2) && // Check Distance between points
...
bool BitMap::checkHistogram(int x1, int y1, int x2, int y2, int _x1, int _y1, int _x2, int _y2)
{
        ...
        (y1 <= y2))
    {
            ...
            (y2 < this->getWidth()))
        {
            ...
            for (int r = x1; r <= x2; r++)
            {
                for (int c = y1; c <= y2; c++)
                {
                    if (((int)this->bmpHandle.getStreamHandle().getLeftBmp().grayscale_img[r][c] >= 0) &&
                        ((int)this->bmpHandle.getStreamHandle().getLeftBmp().grayscale_img[r][c] < 25))
                    {
                        left_hist[0]++;
                    }
                    else if (((int)this->bmpHandle.getStreamHandle().getLeftBmp().grayscale_img[r][c] >= 25) &&
                             ((int)this->bmpHandle.getStreamHandle().getLeftBmp().grayscale_img[r][c] < 50))
                    {
                        left_hist[1]++;
                             ...
                             ((int)this->bmpHandle.getStreamHandle().getLeftBmp().grayscale_img[r][c] < 75))
                    {
                        left_hist[2]++;
                    }
                    else if (((int)this->bmpHandle.getStreamHandle().getLeftBmp().grayscale_img[r][c] >= 75) &&
                             ((int)this->bmpHandle.getStreamHandle().getLeftBmp().grayscale_img[r][c] < 100))
                    {
                        left_hist[3]++;
                             ...
                             ((int)this->bmpHandle.getStreamHandle().getLeftBmp().grayscale_img[r][c] < 125))
                    {
                        left_hist[4]++;
                    }
                    else if (((int)this->bmpHandle.getStreamHandle().getLeftBmp().grayscale_img[r][c] >= 125) &&
                             ((int)this->bmpHandle.getStreamHandle().getLeftBmp().grayscale_img[r][c] < 150))
                    {
                        left_hist[5]++;
                             ...
                             ((int)this->bmpHandle.getStreamHandle().getLeftBmp().grayscale_img[r][c] < 175))
                    {
                        left_hist[6]++;
                    }
                    else if (((int)this->bmpHandle.getStreamHandle().getLeftBmp().grayscale_img[r][c] >= 175) &&
                             ((int)this->bmpHandle.getStreamHandle().getLeftBmp().grayscale_img[r][c] < 200))
                    {
                        left_hist[7]++;
                    }
                    else if (((int)this->bmpHandle.getStreamHandle().getLeftBmp().grayscale_img[r][c] >= 200) &&
                             ((int)this->bmpHandle.getStreamHandle().getLeftBmp().grayscale_img[r][c] < 225))
                    {
                        left_hist[8]++;
                    }
                    else if (((int)this->bmpHandle.getStreamHandle().getLeftBmp().grayscale_img[r][c] >= 225) &&
                             ((int)this->bmpHandle.getStreamHandle().getLeftBmp().grayscale_img[r][c] <= 255))
                    {
                        left_hist[9]++;
                    }
                }
            ...
            for (int r = _x1; r <= _x2; r++)
            {
                for (int c = _y1; c <= _y2; c++)
                {
                    if (((int)this->bmpHandle.getStreamHandle().getRightBmp().grayscale_img[r][c] >= 0) &&
                        ((int)this->bmpHandle.getStreamHandle().getRightBmp().grayscale_img[r][c] < 25))
                    {
                        right_hist[0]++;
                    }
                    else if (((int)this->bmpHandle.getStreamHandle().getRightBmp().grayscale_img[r][c] >= 25) &&
                             ((int)this->bmpHandle.getStreamHandle().getRightBmp().grayscale_img[r][c] < 50))
                    {
                        right_hist[1]++;
                             ...
                             ((int)this->bmpHandle.getStreamHandle().getRightBmp().grayscale_img[r][c] < 75))
                    {
                        right_hist[2]++;
                    }
                    else if (((int)this->bmpHandle.getStreamHandle().getRightBmp().grayscale_img[r][c] >= 75) &&
                             ((int)this->bmpHandle.getStreamHandle().getRightBmp().grayscale_img[r][c] < 100))
                    {
                        right_hist[3]++;
                             ...
                             ((int)this->bmpHandle.getStreamHandle().getRightBmp().grayscale_img[r][c] < 125))
                    {
                        right_hist[4]++;
                    }
                    else if (((int)this->bmpHandle.getStreamHandle().getRightBmp().grayscale_img[r][c] >= 125) &&
                             ((int)this->bmpHandle.getStreamHandle().getRightBmp().grayscale_img[r][c] < 150))
                    {
                        right_hist[5]++;
                             ...
                             ((int)this->bmpHandle.getStreamHandle().getRightBmp().grayscale_img[r][c] < 175))
                    {
                        right_hist[6]++;
                    }
                    else if (((int)this->bmpHandle.getStreamHandle().getRightBmp().grayscale_img[r][c] >= 175) &&
                             ((int)this->bmpHandle.getStreamHandle().getRightBmp().grayscale_img[r][c] < 200))
                    {
                        right_hist[7]++;
                    }
                    else if (((int)this->bmpHandle.getStreamHandle().getRightBmp().grayscale_img[r][c] >= 200) &&
                             ((int)this->bmpHandle.getStreamHandle().getRightBmp().grayscale_img[r][c] < 225))
                    {
                        right_hist[8]++;
                    }
                    else if (((int)this->bmpHandle.getStreamHandle().getRightBmp().grayscale_img[r][c] >= 225) &&
                             ((int)this->bmpHandle.getStreamHandle().getRightBmp().grayscale_img[r][c] <= 255))
                    {
                        right_hist[9]++;
                    }
                }
            ...
            for (int i = 0; i < 10; i++)
            {
                // cout << this->bmpHandle.abs(left_hist[i] - right_hist[i]) << ",";
