...
void BmpHandler::applyCDFOnImages(bool modify)
{
    if (modify)
    {
        ...
        // Calculate Histogram of left image

        for (int i = 0; i < this->streamHandle.getLeftBmp().getImgHeight(); i++)
        {
            for (int j = 0; j < this->streamHandle.getLeftBmp().getImgWidth(); j++)
            {
                hist[this->streamHandle.getLeftBmp().grayscale_img[i][j]]++;
            }
        }

        // Calculate CDF for left image

        int cdf[256];
        cdf[0] = hist[0]; // BASE CASE
        for (int i = 1; i < 256; i++)
        {
            cdf[i] = hist[i] + cdf[i - 1];
        }

        // Apply CDF on Left Image

        for (int i = 0; i < this->streamHandle.getLeftBmp().getImgHeight(); i++)
        {
            for (int j = 0; j < this->streamHandle.getLeftBmp().getImgWidth(); j++)
            {
                this->streamHandle.getLeftBmp().grayscale_img[i][j] = cdf[this->streamHandle.getLeftBmp().grayscale_img[i][j]];
            }
        }

        // Reset the hist and cdf buffer
        ...
        // Calculate histogram of right image

        for (int i = 0; i < this->streamHandle.getRightBmp().getImgHeight(); i++)
        {
            for (int j = 0; j < this->streamHandle.getRightBmp().getImgWidth(); j++)
            {
                hist[this->streamHandle.getRightBmp().grayscale_img[i][j]]++;
            }
        }

        // Calculate CDF for right image
        ...
        // Apply CDF on Right Image

        for (int i = 0; i < this->streamHandle.getRightBmp().getImgHeight(); i++)
        {
            for (int j = 0; j < this->streamHandle.getRightBmp().getImgWidth(); j++)
            {
                this->streamHandle.getRightBmp().grayscale_img[i][j] = cdf[this->streamHandle.getRightBmp().grayscale_img[i][j]];
            }
        }
    }
    else
    {
        throw runtime_error("CDF has not been implemented for RGB images yet");
    }
}
